...
import { subscribeToUser, isFriend, unsubscribeFromUser } from '../functions/request';

function ProfilecardThree(props) {
    const mainId = useSelector(state => state.user.data.payload.id);
    const [subscribed, setSubscribed] = useState(false);
    const user = props.user;

    const subscribe = async () => {
        const result = await subscribeToUser(user.id)
        if(!result.statusCode) setSubscribed(true);
    }

    const unsubscribe = async () => {
        const result = await unsubscribeFromUser(user.id)
        if(!result.statusCode) setSubscribed(false);
    }

    useEffect(() => {
        if(mainId === user.id) return;
        isFriend(user.id).then(res => setSubscribed(res));
    }, [])

    return (
        <div className="card w-100 border-0 p-0 bg-white shadow-xss rounded-xxl">
            <div className="card-body h250 p-0 rounded-xxl overflow-hidden bg-image-cover bg-image-center m-3" style={{height: "300px", backgroundImage: user.background_image ? `url(${user.background_image.url})` : `url(/assets/images/default_background.jpg)`}} ></div>
            <div className="card-body p-0 position-relative">
                <figure className="avatar position-absolute w100 z-index-1" style={{top:'-40px',left:'30px'}}><img src={user.avater ? `${process.env.REACT_APP_DOMAIN}${user.avater.url}` : "/assets/images/profile.png"} alt="avater" className="float-right p-1 bg-white rounded-circle w-100" /></figure>
                <h4 className="fw-700 font-sm mt-2 mb-lg-5 mb-4 pl-15">@{user.username} <span className="fw-500 font-xssss text-grey-500 mt-1 mb-3 d-block">{user.display_name}</span></h4>
                <div className="d-flex align-items-center justify-content-center position-absolute-md right-15 top-0 me-2">
                    {!subscribed && mainId !== user.id && <a onClick={subscribe} href="#" className="d-none d-lg-block bg-success p-3 z-index-1 rounded-3 text-white font-xsssss text-uppercase fw-700 ls-3">Subscribe</a>}
                    {subscribed && mainId !== user.id && <a onClick={unsubscribe} href="#" className="d-none d-lg-block bg-warning p-3 z-index-1 rounded-3 text-white font-xsssss text-uppercase fw-700 ls-3">Unsubscribe</a>}
                    {mainId !== user.id && <a href={`/chat/${user.id}`} className="d-none d-lg-block bg-greylight btn-round-lg ms-2 rounded-3 text-grey-700"><i className="feather-mail font-md"></i></a>}
